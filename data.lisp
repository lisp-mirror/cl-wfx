  ...
  (dolist (definition definitions)
    (let ((destinations (or (getf definition :destinations)
      ...
      (when (find destination destinations :test 'equalp)
        (let ((document-document-type (getf definition :document-type)))
          (when document-document-type
            (let ((elements))
              (dolist (element (getf document-document-type :elements))
              ...
              (add-document-type
               ...
               (make-instance
                ...
                :label (getf document-document-type :label)
                :elements elements
                :destinations
            ...
            (when collection-def
              (let ((document-type (get-document-type (get-store universe store-name)
                ...
                (unless document-type
                  #|
                  (break "?~A~%~S" (getf collection-def :document-type) collection-def)
                  (break "??~A~%~A" (getf collection-def :document-type)
...
(defun find-type-def (system name)
  (let ((definitions (data-definitions system)))
    ...
    (dolist (def definitions)
      (let ((col (digx def :document-type)))
...
(defun wfx-query-data (collection &key query)
  (let ((documents)
    ...
    (naive-impl:set-query-cache :context *context*)
