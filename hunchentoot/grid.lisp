...
(defun complex-type (element)
  (if (listp (digx element :concrete-type))
      (or (digx element :concrete-type :complex-type)
          (digx element :concrete-type :type))
      (digx element :concrete-type)))
...
(defun check-top-level-p (document-type)
  (when document-type
    (when (getcx document-type :concrete-type)
      (digx (getcx document-type :concrete-type) :top-level-p))))
...
(defmethod render-input-val-x ((type (eql :value-list)) element document
  ...
  (let* ((name (getx element :name))
         (list (or (and (digx element :concrete-type :values-lambda)
                        (cond ((equalp (first (digx element :concrete-type :values-lambda))
                                       'cl-wfx::get-named-list-sorted-values)
                               (eval% (digx element :concrete-type :values-lambda)))
                              ((equalp (first (digx element :concrete-type :values-lambda))
                                       'cl:lambda)
           ...
           :query (lambda (documentx)
                    (if (getx element :filter)
                        (funcall
                         ...
                         documentx
                         edit-document
                         (getcx (digx element :concrete-type :type) :edit-object))
                        t))))

        (container-accessors (digx element :concrete-type :container-accessor))
        (accessors (digx element :concrete-type :accessor))
        (list))

    (when (not (digx element :concrete-type :container-fetch))

      (dolist (list-document list-containers)

        (setf list (pushnew (accessor-value list-document container-accessors) list))))

    (when (digx element :concrete-type :container-fetch)

      (dolist (list-document list-containers)
        (setf list
              (append list (funcall
                            (eval% (digx element :concrete-type :container-fetch))
...
(defun find-in-hierarchy (document-type hierarchy)
  (dolist (hierarchy-document hierarchy)
    (when (string-equal (getx hierarchy-document :document-type)
                        document-type)
    ...
    (with-html-string
      (cl-who:str
       (render-dropdown name selected list
                        :key-func 'cl-naive-store.naive-documents:document-hash
                        :value-func (lambda (document)
                                      (accessor-value document accessors)))))))
...
(defun render-grid-buttons (document-type document hierarchy)
  (let ((permissions (getx (context-spec *context*) :permissions)))
    (with-html-string
      ...
      (dolist (action (getx (getcx document-type :document-type) :document-actions))
                            ...
                            (dolist (element elements)
                              (when (and (digx  element :attributes :display)
                                ...
                                (when (equalp (complex-type element)
                                  ...
                                  (when (document-type-access-p (digx  element :concrete-type :type))
                                    (cl-who:str
...
(defun grid-js-render-new (document-type hierarchy)
  (let ((active-page (getcx document-type :active-page))
    ...
    (setf documents (append hierarchy (list (list :document-type document-type
                                                  :document 0))))
  ...
  (when (string-equal (frmt "~A" (getcx document-type :expand-id))
    ...
    (with-html-string
      (:div :class "row card-columns"
            (dolist (sub subs)
              (let* ((sub-data-spec (digx sub :concrete-type :document-type)))
                (setf (getcx sub-data-spec :parent-document) document)

                (setf (getcx sub-data-spec :collection-name)
                      (digx sub :concrete-type :collection))

                (unless (getcx sub-data-spec :document-type)
                  (setf (getcx sub-data-spec :document-type)
                        (find-type-defenition *system* (digx sub :concrete-type :collection)
                                              sub-data-spec))

                  (setf (getcx sub-data-spec :elements)
                        (digx (getcx sub-data-spec :document-type)
                              :elements)))
      ...
      (let ((elements (getcx document-type :elements))
        ...
        (dolist (element elements)
          (when (sub-grid-p element)
            ...
            (when (document-type-access-p (getx (digx element :concrete-type)
                                                :type))
        ...
        (dolist (document documents)
          (let ((document-hierarchy (append hierarchy (list (list :document-type
                                                                  document-type
                                                                  :document document)))))
                ...
                (or
                 ...
                 (getcx document-type
                        (intern (string-upcase
                                 (frmt "~A-filter" (getx element :name))))))))
          (when filter-term
            (when (digx element :concrete-type)
              (when (sub-grid-p element)
                (let ((accessor (digx element :concrete-type :accessor)))
...
(defmethod action-handler ((action (eql :document-action))
  ...
  (let* ((document-type (getcx (parameter "document-type") :document-type))
...
(defun set-type-context (collection document-type)
  ...
  (unless (getcx document-type :document-type)
    (setf (getcx document-type :document-type)
          (find-type-defenition *system* (digx collection :collection :name) document-type))
    (setf (getcx document-type :elements)
          (digx (getcx document-type :document-type) :elements))))
...
(defun set-grid-context (collection-name document-type)
  (unless (gethash :collection-name (cache *context*))
    ...
    (setf  (gethash :document-type (cache *context*)) document-type)
  ...
  (when (context-access-p (context-spec *context*))
    (let* ((collection (if (not (gethash :collection-name (cache *context*)))
                           (find-collection-def *system*
                                                collection-name)))
           (document-type (or (gethash :document-type (cache *context*))
                              (and collection
                                   (digx collection :collection :document-type)))))
  ...
  (let* ((collection-name id)
    ...
    (dolist (store stores)
      (let ((collection (get-collection
        ...
        (when collection
          (cl-naive-store.naive-core:sanitize-data-file collection)))))
...
(defun get-child (elements parent-document document-type hash)
  ...
  (dolist (element elements)
    ...
    (when (sub-grid-p element)
      ...
      (when (string-equal document-type (digx element :concrete-type :document-type))
...
(defun ajax-auto-complete (&key id from-ajax)
  ...
  (let* ((document-type (parameter "document-type"))
    ...
    (when element
      ...
      (let* ((accessors (digx element :concrete-type :accessor))
             (list))
      ...
      (if (> (length hierarchy) 1)
          (let ((document)
            ...
            (dolist (child (cdr hierarchy))
              ...
              (setf edit-objects
                    (push (list :document-type document-type
      ...
      (if (> (length hierarchy) 1)
          ...
          (progn
            ...
            (render-grid-edit root-type elements root-document nil nil
                              (list (list :document-type document-type
      ...
      (when elements
        ...
        (when (digx (getcx document-type :document-type) :document-type :client-validation)
          (let ((valid (funcall (eval% (digx (getcx document-type :document-type)
                                             :document-type :client-validation))
                                root-document
                                edit-document)))
            (unless (first valid)
...
(defmethod action-handler ((action (eql :delete))
  ...
  (multiple-value-bind (root-document parent-slot parent-document edit-document)
    ...
    (unless (and edit-document parent-slot)
      ...
      (persist-document (document-collection root-document) root-document)
      (cl-naive-store.naive-core::remove-document (document-collection root-document) root-document))))
